fitGAMtoEffort <-
function(input = lllf, which.flag='Japan',which.effort='NO.HOOKS',start.year=1950,end.year=2010,kk=6,which.gam='mgcv')
{
...
if(which.gam=='mgcv')
  {
bbs <- 'cr'
h1 <- gam(eff1~te(longitude,latitude,k=kk,bs=bbs)+te(trend,k=kk,bs=bbs)+sin1+cos1+sin2+cos2+sin3+cos3+sin4+cos4+sin5+cos5+cos6,family=quasipoisson(link="log"),method="REML",data=input)
#print(summary(h1))
}
else{
  
  registerDoMC(cores=5)
  h1 <- gam(eff1~lo(longitude,latitude)+lo(trend)+sin1+cos1+sin2+cos2+sin3+cos3+sin4+cos4,data=input,family=negbin(1))

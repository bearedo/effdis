...
```{r temporal confounding Japan and Chinese Taipei,include=TRUE,echo=FALSE}
par(mfrow=c(2,2),mar=c(2,2,2,2),oma=c(1,1,1,1))
out <- sqlQuery(chan,"SELECT yearc AS year, trend, timeperiodid AS month, flagname, region, geargrpcode,longitude,latitude, eff1
FROM t2ce
WHERE region ='AT' AND timeperiodid < 13 AND eff1type='NO.HOOKS' AND flagname ='Japan' OR flagname = 'Chinese Taipei' OR flagname = 'Brasil' OR flagname = 'U.S.A.' OR flagname = 'China P.R.' AND geargrpcode ='LL';")
...
three.d.effort.by.year.r(tdata=out,what.year='2006',what.flag='China P.R.',scaling.f=100000)  
```
